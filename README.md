# Buffett Lens · 巴菲特视角个股建仓判断

输入美股 / A股 / 港股代码（如 AAPL / 600519.SS / 0700.HK），立刻判断「现在能不能建仓」。核心逻辑：**巴菲特式质量门槛 + 内在价值安全边际 + 建议买入价 + 公司介绍**。
...
- **完整信息**：市场/货币/行业/细分行业（行业中文名）+ 公司简介 + 官网/总部
...
代码格式：美股 `AAPL`；沪市A股 `600519.SS`；深市A股 `000858.SZ`；港股 `0700.HK`（腾讯）。
...
- 本工具为巴菲特式定量参考，不构成投资建议；护城河、管理层等定性因素需自行判断。
